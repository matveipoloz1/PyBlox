import os, sys, json, hashlib, threading, time, ssl, socket, io, queue, subprocess, tempfile
...
class PyBlox:
    ...
    def _set_user_speaking(self, username, sp):
        ...
        try:
            ...
            c.itemconfig(w['ring_id'], outline="#4caf50" if sp else "#cfd8dc")
        except Exception: pass
    ...
    def show_menu(self):
        ...
        for w in self.root.winfo_children(): w.destroy()
        f=tk.Frame(self.root); f.pack(expand=True)
        tk.Label(f, text=t('hello', name=self.username),
    ...
    def _enter_server_ready(self, name):
        ...
        # ---- Файловый сервер (у каждого свой) ----
        if FILES_OK:
            try:
    ...
    def _create_avatar_widget(self, username):
        ...
        ring=cvs.create_oval(1,1,71,71, outline="#cfd8dc", width=3)
    ...
    def _create_video_widget(self, key):
        ...
        tk.Label(c, text=f"{user}  {icon}", bg="#1b1b1b", fg="#eceff1",
